...
def welcome():
    ...
    return (
        f"<h1>Welcome to Tamica's Climate App API!</h1>"
        f"This is a Flask API for Climate Analysis .<br/><br/><br/>"
        f"<img src='https://preview.redd.it/qmqiarwympe91.png?width=749&format=png&auto=webp&v=enabled&s=183fc18537c64cdc48b2b09bc8a029a83ab06841'/ >"
        ...
        f"<h2>Hyperlinks for each available route, click the link to see explore:</h2>"
        f"<ol><li><a href=http://127.0.0.1:5000/api/v1.0/precipitation>"
        f"Precipitation amounts by date (for most recent year of data available)</a></li><br/><br/>"
        f"<li><a href=http://127.0.0.1:5000/api/v1.0/stations>"
        f"Detailed list of weather stations</a></li><br/><br/>"
        f"<li><a href=http://127.0.0.1:5000/api/v1.0/tobs>"
        f"Recorded temperatres for the last 12 months</a></li><br/><br/>"
        f"<li><a href=http://127.0.0.1:5000/api/v1.0/2017-08-23>"
        f"Enter a start date (YYYY-MM-DD), to calculate the minimum, average, and maximum temperatures for all dates greater than and equal to the start date</a></li><br/><br/>"
        f"<li><a href=http://127.0.0.1:5000/api/v1.0/2016-08-23/2017-08-23>"
        f"Enter start and end dates (YYYY-MM-DD), to calculate the minimum, average, and maximum temperatures for dates between the start and end date</a></li></ol><br/>"
...
def precipitation():
    ...
    # Calculate the date 1 year prior to the last data point in the DB.
...
def tobs():
    ...
    # Calculate the date 1 year ago from the last data point in the DB.
    last_measurement_data_point_tuple = session.query(
        Measurement.date).order_by(Measurement.date.desc()).first()
    (latest_date, ) = last_measurement_data_point_tuple
    latest_date = dt.datetime.strptime(latest_date, '%Y-%m-%d')
